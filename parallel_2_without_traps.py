...
if __name__ == "__main__":
    ...
    plt.figure(3)
    plt.plot(time_plot[0:], sum(np.array(Flow))[0:] / 1.4e-5, 'g--', linewidth=2)  # строим исходный график
    # plt.title(sample[ii])
    plt.title("Численная десорбционная кривая")
    plt.xlabel("Time")
    plt.ylabel("Flow")
    plt.grid()
    plt.savefig('modeling_7.png', dpi=1000)
    out = input()
